...
// Copied from rill-sync, where it was measured on two devices. Two changes for
// Rill Chamber: its own magic, so a chamber ensemble never mixes with a Mallet
// or Drums one, and sharedMicros(), a timeline every device agrees on.
//
...
constexpr uint32_t magic = 0x524c4348;  // "RLCH"
...
class Clock {
 public:
  ...
  // The ensemble's shared timeline in microseconds, read at local time `now`:
  // the conductor's beat count times the period, less the time to the next
  // beat. Every device reads the same value at the same instant, to within the
  // clock's error, so events placed on it happen together.
  int64_t sharedMicros(int64_t now) const {
    return int64_t(beatIndex_) * int64_t(beatPeriod_) - (nextBeat_ - now);
  }
  ...
  // How often this device took a conductor's grid outright, and how often it
  // took over after hearing nothing: each one moves the shared timeline.
  uint32_t joins() const { return joins_; }
  uint32_t takeovers() const { return takeovers_; }
  ...
  void receive(const Packet& p, int64_t now) {
    ...
    if (changed) {
      ...
      // beat ahead, and jumping the shared timeline by a beat, until the next
      // packet put it back. Rill Sync only showed the count, so it never told.
