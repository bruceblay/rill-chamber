...
const stages=[],parts=[],pieces=[],sections=[];
for(const slug of SLUGS){
 ...
 for(const part of piece.devices){
  ...
  parts.push(`{${text(part.name)},${VOICES[part.voice]},${sample},${part.transpose??0},${float(part.level??1.3)},${float(part.rate??1)},${text(part.main)},${stageStart},${part.stages.length}}`);
 }
 pieces.push(`{${text(piece.title)},${text(piece.composer)},${Math.round(piece.period*1e6)},${piece.cycle??piece.pattern.length},${piece.featured??-1},${partStart},${piece.devices.length},${sectionStart},${(piece.sections??[]).length}}`);
}
