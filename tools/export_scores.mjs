...
// Process pieces come from phase-scores.js: every one the lab has for four
// devices or fewer. Composed ones come from public/bach, played on a
// harpsichord synthesized as the lab's is.
const SLUGS=['piano-phase','clapping','violin-phase','music-in-fifths','moutons'];
...
const VOICES={piano:'Voice::Piano',sample:'Voice::Sample',marimba:'Voice::Marimba',xylophone:'Voice::Xylophone',glass:'Voice::Glass',strings:'Voice::Strings',organ:'Voice::Organ'};
// Held voices sound for a share of the pulse (the stage's hold); struck ones ring.
const HELD=new Set(['strings','organ']);
...
for(const slug of SLUGS){
 ...
 for(const part of piece.devices){
  ...
  for(const stage of part.stages){
   ...
   stages.push(`{${float(stage.cycles)},${MODES[stage.mode]},${stage.from!==undefined},${float(stage.from??0)},${float(stage.to??0)},${stage.rate!==undefined},${float(stage.rate??1)},${!!stage.restart},${!!stage.once},${float(stage.slips??0)},${pattern},${text(stage.label)},${text(stage.main)},${text(stage.word)},${float(HELD.has(part.voice)?stage.hold??0.9:0)}}`);
  }
 ...
 // One collection for the process pieces; the byline names whose they are after.
 pieces.push(`{${text(piece.title)},${text(piece.composer)},${text(`After ${piece.composer.split(' ').at(-1)}`)},"Process",0.0f,${Math.round(piece.period*1e6)},${piece.cycle??piece.pattern.length},${piece.featured??-1},${partStart},${piece.devices.length},${sectionStart},${(piece.sections??[]).length}}`);
}
...
for(const slug of SLUGS){
 const piece=PIECES[slug];
 for(const part of piece.devices){
  // A part that slips at random has no fixed note times: its span is marked
  // so the test only checks that it plays through.
  if(part.stages.some(stage=>stage.slips)){spans.push(`{${vectors.length},RANDOM}`);continue;}
...
struct Span { unsigned start, count; };\nconstexpr unsigned RANDOM = ~0u;  // a part that slips at random
